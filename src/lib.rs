    ...
    D: DelayNs,
{
    ...
    pub fn begin_with_dotsize(
        ...
        // Retry first command for more reliable initialization
        let mut last_err = None;
        for _attempt in 0..3 {
            match self.command(LCD_FUNCTIONSET | self.display_function) {
                Ok(_) => {
                    last_err = None;
                    break;
                }
        ...
        // Detect and initialize RGB backlight chip with retries
        let mut rgb_v5_detected = false;
        for _attempt in 0..3 {
            self.delay.delay_ms(10);
            // Try v5 address first
            if self.i2c.write(RGB_ADDRESS_V5, &[]).is_ok() {
                rgb_v5_detected = true;
                break;
            }
        }
        ...
        if rgb_v5_detected {
            ...
            self.set_reg(0x00, 0x07).ok(); // Reset - ignore errors
            self.delay.delay_ms(200);
            self.set_reg(0x04, 0x15).ok(); // Set all LEDs always on
        } else {
            // Try v4 address
            let mut rgb_v4_detected = false;
            for _attempt in 0..3 {
                self.delay.delay_ms(10);
                if self.i2c.write(RGB_ADDRESS, &[]).is_ok() {
                    rgb_v4_detected = true;
                    break;
                }
            }
            ...
            if rgb_v4_detected {
                ...
                self.set_reg(REG_MODE1, 0).ok();
                self.set_reg(REG_OUTPUT, 0xFF).ok();
                self.set_reg(REG_MODE2, 0x20).ok();
            }
        }

        // Set default white backlight (ignore errors if RGB not available)
        let _ = self.set_rgb(255, 255, 255);
    ...
    pub fn set_rgb(&mut self, r: u8, g: u8, b: u8) -> Result<(), LcdError<E>> {
        // v5 uses different register addresses than v4
        if self.rgb_addr == RGB_ADDRESS_V5 {
            self.set_reg(0x06, r)?;  // v5 red register
            self.set_reg(0x07, g)?;  // v5 green register
            self.set_reg(0x08, b)?;  // v5 blue register
        } else {
            self.set_reg(REG_RED, r)?;    // v4: 0x04
            self.set_reg(REG_GREEN, g)?;  // v4: 0x03
            self.set_reg(REG_BLUE, b)?;   // v4: 0x02
        }
        Ok(())
    }
